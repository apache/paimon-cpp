...
pub unsafe extern "C" fn paimon_lance_writer_write(
    ...
    catch_panic(-1, || {
        ...
        if writer.failed {
            return fail("cannot write after Lance writer failed");
        }
...
pub unsafe extern "C" fn paimon_lance_writer_tell(
    ...
    catch_panic(-1, || {
        ...
        let writer = unsafe { &mut *writer };
        if writer.failed {
            return fail("cannot get position after Lance writer failed");
        }
        match block_on(async {
            writer
                .writer
...
pub unsafe extern "C" fn paimon_lance_writer_finish(
    ...
    catch_panic(-1, || {
        ...
        if writer.failed {
            return fail("cannot finish after Lance writer failed");
        }
        writer.failed = true;
        match block_on(async {
            writer
...
mod tests {
    ...
    unsafe fn write_batch(writer: *mut PaimonLanceWriter) -> i32 {
        let (mut array, mut schema) = to_ffi(&test_array().to_data()).unwrap();
        unsafe {
            paimon_lance_writer_write(
                writer,
    ...
    #[test]
    fn writer_stays_failed_after_panic() {
        let status = Command::new(std::env::current_exe().unwrap())
            .args([
                "--exact",
                "writer::tests::writer_stays_failed_after_panic_child",
                "--nocapture",
            ])
            .env("PAIMON_LANCE_PANIC_TEST_CHILD", "1")
            .env("LANCE_FILE_WRITER_MAX_PAGE_BYTES", "0")
            .status()
            .unwrap();
        assert!(status.success());
    }

    fn run_writer_panic_case() {
        if std::env::var_os("PAIMON_LANCE_PANIC_TEST_CHILD").is_none() {
            return;
        }
        let path = std::env::temp_dir().join(format!(
            "paimon-lance-writer-panic-{}.lance",
            std::process::id()
        ));
        ...
        unsafe {
            assert_eq!(
                paimon_lance_writer_open(uri.as_ptr(), ptr::null(), ptr::null(), 0, &mut writer,),
                0
            );
            assert!(!writer.is_null());
            assert_eq!(write_batch(writer), 0);

            let panic_hook = std::panic::take_hook();
            std::panic::set_hook(Box::new(|_| {}));
            let mut row_count = 0;
            assert_eq!(paimon_lance_writer_finish(writer, &mut row_count), -1);
            std::panic::set_hook(panic_hook);
            assert!(CStr::from_ptr(paimon_lance_last_error())
                .to_str()
                .unwrap()
                .starts_with("Lance panicked:"));

            assert_eq!(paimon_lance_writer_finish(writer, &mut row_count), -1);
            assert_eq!(
                CStr::from_ptr(paimon_lance_last_error()).to_str().unwrap(),
                "cannot finish after Lance writer failed"
            );
            assert_eq!(write_batch(writer), -1);
            assert_eq!(
                CStr::from_ptr(paimon_lance_last_error()).to_str().unwrap(),
                "cannot write after Lance writer failed"
            );
            paimon_lance_writer_free(writer);
        }
        let _ = std::fs::remove_file(path);
    }

    #[test]
    fn writer_stays_failed_after_panic_child() {
        run_writer_panic_case();
    }
}
