...
Result<std::shared_ptr<GlobalIndexReader>> LuminaGlobalIndex::CreateReader(
    ...
    PAIMON_ASSIGN_OR_RAISE(std::unique_ptr<LuminaIndexSearcher> searcher,
                           LuminaIndexSearcher::Open(lumina_options, index_info, in, lumina_pool));
    return std::make_shared<LuminaIndexReader>(std::move(searcher));
}
...
LuminaIndexReader::LuminaIndexReader(std::unique_ptr<LuminaIndexSearcher>&& searcher)
...
Result<std::shared_ptr<ScoredGlobalIndexResult>> LuminaIndexReader::VisitVectorSearch(
    ...
    PAIMON_ASSIGN_OR_RAISE(::lumina::api::LuminaSearcher::SearchResult search_result,
                           searcher_->Search(vector_search));
