...
/// Legacy serializer for `DataFileMeta` before column sequence numbers were introduced.
class DataFileMetaWriteColsLegacySerializer
    : public ObjectSerializer<std::shared_ptr<DataFileMeta>> {
 public:
    static const std::shared_ptr<arrow::DataType>& DataType();

    explicit DataFileMetaWriteColsLegacySerializer(const std::shared_ptr<MemoryPool>& pool)
