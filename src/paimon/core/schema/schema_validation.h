...
class SchemaValidation {
 public:
    ...
    ///
    /// @param file_system The file system the table is being created on, already resolved, or
    ///        null to resolve it from the schema's options. Nothing here reads through it today;
    ///        passing it keeps validation from resolving a different one than creation uses.
    ...
    ///        through the options. Spares resolving `file-system` a second time, which would fail
    ///        for a caller that handed its own over rather than naming one.
