...
TEST(LazyConcatBatchReaderTest, TestAReaderIsOpenedOnlyWhenItIsReached) {
    ...
    LazyConcatBatchReader reader(SourcesOf(std::move(factories)), GetArrowPool(GetDefaultPool()));
    ASSERT_EQ(opened, 0);
...
TEST(LazyConcatBatchReaderTest, TestAFailureNamesTheFileItCameFrom) {
    ...
    LazyConcatBatchReader open_reader(SourcesOf(std::move(failing_open)),
    ...
    LazyConcatBatchReader read_reader(SourcesOf(std::move(failing_read)),
                                      GetArrowPool(GetDefaultPool()));
