...
///
/// Most of `FileStoreCommit` is about snapshots and manifests, which a format table keeps none
/// of, so each such call is refused rather than quietly doing nothing. `RowIdCheckConflict()` is
/// the exception, returning a reference with no way to report one.
