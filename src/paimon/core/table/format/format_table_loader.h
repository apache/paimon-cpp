...
class FormatTableLoader {
 public:
    ...
    ///        in the same entry point takes it, or the two dispatch on different schemas. Anything
    ///        but the main branch is refused once the schema turns out to be a format table's,
    ///        which has no branches to read.
    /// @param options Options given at the call, which win over the ones the schema stored.
    /// @param specific_table_schema A schema the caller already holds, as json, which saves
    ///        reading one from under the path. Used only on the main branch, as on the managed
    ///        read path.
    /// @param schema_manager The manager to read the schema through, or null to use one of this
    ///        call's own. A caller that has one must pass it: a manager caches what it read.
    /// @param table_schema_out Set to the schema this read, or null when there is none, so that a
    ///        caller falling back to the managed path does not read it twice.
