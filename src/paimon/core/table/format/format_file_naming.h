...
/// and the name are hidden, the Hive-style convention for uncommitted output that a scan skips.
///
/// TODO(SteNicholas): support publishing without a rename. On an object store a rename is a copy
/// followed by a delete, so a commit pays for the data twice and cannot undo a partly finished
/// publish - which is why `FormatTableCommit`'s rollback is best-effort. Java's
/// `MultiPartUploadTwoPhaseOutputStream` writes straight to the final key and defers only
/// `CompleteMultipartUpload`, making the publish one atomic call with no copy.
class FormatFileNaming {
 public:
    ...
    static Result<std::unique_ptr<FormatFileNaming>> Create(const std::string& extension,
    ...
    /// staged name carries a uuid of its own, so two writers sharing `_temporary` cannot collide.
