...
/// `FileStoreWrite` and `FileStoreCommit` like any other. It names a staged path rather than
/// files to record in a manifest, so `CommitMessage::Serialize()` refuses it: there is no
/// cross-runtime encoding for one, and a write and its commit belong to the same process.
