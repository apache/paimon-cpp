...
/// A file system that reports one directory gone when it is listed, standing in for a directory
/// another job removed between the listing that found it and the listing that descends into it.
/// Only `ListFileStatus()` is answered that way, since that is where the race lands: the parent
/// has already handed the name out.
class VanishingDirectoryFileSystem : public LocalFileSystem {
 public:
    explicit VanishingDirectoryFileSystem(std::string vanished) : vanished_(std::move(vanished)) {}

    Status ListFileStatus(const std::string& path,
                          std::vector<FileStatus>* file_status_list) const override {
        if (path == vanished_) {
            return Status::NotExist(path + " is gone");
        }
        return LocalFileSystem::ListFileStatus(path, file_status_list);
    }

 private:
    std::string vanished_;
};
...
TEST(FormatFileListingTest, TestMissingRootIsAnErrorButAVanishedSubdirectoryIsNot) {
    ...
    const std::string vanished = dir->Str() + "/gone";
    ASSERT_OK(WriteAt(file_system, vanished + "/b.parquet"));
    std::shared_ptr<FileSystem> vanishing =
        std::make_shared<VanishingDirectoryFileSystem>(vanished);
    ASSERT_OK_AND_ASSIGN(std::vector<std::string> names,
                         ListNames(vanishing, dir->Str(), FormatDataFileListingOptions{}));
    ASSERT_EQ(names, (std::vector<std::string>{"a.parquet"}));

    // What the same tree lists as when nothing goes missing, so that the listing above is short
    // because the directory vanished rather than because there was never anything below it.
    ASSERT_OK_AND_ASSIGN(std::vector<std::string> whole,
                         ListNames(file_system, dir->Str(), FormatDataFileListingOptions{}));
    ASSERT_EQ(whole, (std::vector<std::string>{"a.parquet", "gone/b.parquet"}));
}
