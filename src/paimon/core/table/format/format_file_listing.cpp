        ...
        for (const std::string& directory : level) {
            ...
            for (const FileStatus& child : children) {
                ...
                if (child.IsDir()) {
                    ...
                    // At a partition level, only a directory naming this table's partition is
                    // its data: a caller that deletes what this returns would otherwise remove
                    // files the table never owned. The value-only layout has nothing to check,
                    // since the level alone says which key a directory belongs to.
                    if (children_are_partitions && !options.only_value_in_path &&
                        !options.partition_keys.empty()) {
                        const auto key_index = static_cast<size_t>(depth);
                        if (key_index >= options.partition_keys.size()) {
                            continue;
                        }
                        std::optional<std::pair<std::string, std::string>> key_value =
                            PartitionPathUtils::ExtractPartitionKeyValue(name);
                        if (!key_value || key_value->first != options.partition_keys[key_index]) {
                            continue;
                        }
                    }
                    next.push_back(child.GetPath());
                } else if (!hidden && !children_are_partitions) {
