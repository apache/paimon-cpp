...
TEST(CommitMessageTest, TestDeserializeVersion13GeneratedByJava) {
    // Generated by CommitMessageSerializer::serialize from Apache Paimon Java master.
    std::string data_path = paimon::test::GetDataDir() + "/compatibility/commit_message-v13";
    auto file_system = std::make_shared<LocalFileSystem>();
    auto buffer_length = file_system->GetFileStatus(data_path).value().GetLen();
    ASSERT_GT(buffer_length, 0);

    std::vector<char> buffer(buffer_length, 0);
    ASSERT_OK_AND_ASSIGN(auto in_stream, file_system->Open(data_path));
    ASSERT_OK(in_stream->Read(buffer.data(), buffer.size()));
    ASSERT_OK(in_stream->Close());

    auto pool = GetDefaultPool();
    ASSERT_OK_AND_ASSIGN(std::shared_ptr<CommitMessage> result,
                         CommitMessage::Deserialize(CommitMessage::CurrentVersion(), buffer.data(),
                                                    buffer.size(), pool));
    auto result_message = std::dynamic_pointer_cast<CommitMessageImpl>(result);
    ASSERT_NE(result_message, nullptr);
    ASSERT_EQ(result_message->Partition(),
              BinaryRowGenerator::GenerateRow({std::string("aaaaa")}, pool.get()));
    ASSERT_EQ(result_message->Bucket(), 20);
    ASSERT_EQ(result_message->TotalBuckets(), std::optional<int32_t>(32));
    ASSERT_EQ(result_message->GetNewFilesIncrement().NewFiles().size(), 1);
    ASSERT_TRUE(result_message->GetCompactIncrement().IsEmpty());

    const std::shared_ptr<DataFileMeta>& data_file =
        result_message->GetNewFilesIncrement().NewFiles()[0];
    ...
    ASSERT_OK_AND_ASSIGN(std::string serialized_bytes, CommitMessage::Serialize(result, pool));
    ASSERT_EQ(serialized_bytes, std::string(buffer.data(), buffer.size()));
}
