...
TEST(DataSplitTest, TestDeserializeVersion9GeneratedByJava) {
    // Generated by DataSplit::serialize from Apache Paimon Java master.
    std::string file_name = paimon::test::GetDataDir() + "/compatibility/data_split-v9";
    ...
    ASSERT_NE(result_data_split, nullptr);
    ASSERT_EQ(result_data_split->SnapshotId(), 18);
    ASSERT_EQ(result_data_split->Partition(),
              BinaryRowGenerator::GenerateRow({std::string("aaaaa")}, pool.get()));
    ASSERT_EQ(result_data_split->Bucket(), 20);
    ASSERT_EQ(result_data_split->BucketPath(), "my path");
    ASSERT_EQ(result_data_split->TotalBuckets(), std::optional<int32_t>(32));
    ASSERT_EQ(result_data_split->DataFiles().size(), 1);
    ASSERT_EQ(
        result_data_split->DeletionFiles(),
        (std::vector<std::optional<DeletionFile>>{DeletionFile("deletion_file", 100, 22, 33)}));
    ASSERT_FALSE(result_data_split->IsStreaming());
    ASSERT_FALSE(result_data_split->RawConvertible());

    const std::shared_ptr<DataFileMeta>& data_file = result_data_split->DataFiles()[0];
    ASSERT_EQ(data_file->file_name, "my_file");
    ASSERT_TRUE(data_file->write_cols.has_value());
    ASSERT_EQ(data_file->write_cols.value(), (std::vector<std::string>{"a", "b", "c", "f"}));
    ASSERT_TRUE(data_file->column_max_sequence_numbers.has_value());
    ASSERT_EQ(data_file->column_max_sequence_numbers.value(),
              (std::vector<int64_t>{15, 100, 150, 200}));

    ASSERT_OK_AND_ASSIGN(std::string serialized_bytes, Split::Serialize(result, pool));
    ASSERT_EQ(serialized_bytes, std::string(split_bytes.data(), split_bytes.size()));
}
