...
Result<std::shared_ptr<TableScanResources>> TableScanResources::Create(
    ...
    if (!file_system) {
        return Status::Invalid("table scan resources require a file system");
    }
    ...
    return std::shared_ptr<TableScanResources>(new TableScanResources(
        std::make_unique<Impl>(path, BranchManager::NormalizeBranch(branch), file_system)));
}
...
TableScanResources::Impl::Impl(const std::string& path, const std::string& branch,
    ...
    : path_(path),
      ...
      snapshot_manager_(std::make_shared<SnapshotManager>(
          file_system, path, branch, std::make_shared<SnapshotManager::SnapshotCache>())) {}
...
Status TableScanResources::Impl::Validate(
    ...
    if (specific_file_system != file_system_) {
