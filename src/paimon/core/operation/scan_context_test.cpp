...
TEST(ScanContextTest, TestTableResourcesValidationAndReset) {
    ...
    ASSERT_OK_AND_ASSIGN(auto resources,
                         TableScanResources::Create("table/", fs, "dev", GetDefaultPool()));
...
TEST(ScanContextTest, TestInvalidTableResources) {
    auto fs = std::make_shared<MockFileSystem>();
    ASSERT_NOK(TableScanResources::Create("", fs, "main", GetDefaultPool()));
    ASSERT_NOK(TableScanResources::Create("table", nullptr, "main", GetDefaultPool()));
    ASSERT_NOK(TableScanResources::Create("table", fs, "main", nullptr));
    ASSERT_NOK(TableScanResources::Create("table", fs, "../other", GetDefaultPool()));
    ASSERT_NOK_WITH_MSG(TableScanResources::Create("table$ro", fs, "main", GetDefaultPool()),
                        "physical table path");
    ASSERT_OK_AND_ASSIGN(auto resources,
                         TableScanResources::Create("table", fs, "", GetDefaultPool()));
    ScanContextBuilder builder("table");
