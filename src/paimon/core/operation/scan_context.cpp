...
std::shared_ptr<FileSystem> ScanContext::GetSpecificFileSystem() const {
    if (specific_file_system_) {
        return specific_file_system_;
    }
    return table_resources_ ? TableScanResourcesAccess::Get(*table_resources_).file_system_
                            : nullptr;
}
...
Result<std::unique_ptr<ScanContext>> ScanContextBuilder::Finish() {
    ...
    if (impl_->table_resources_) {
        ...
        PAIMON_RETURN_NOT_OK(
            resources.Validate(impl_->path_, branch->second, impl_->specific_file_system_));
    }
    ...
    auto ctx = std::make_unique<ScanContext>(
        ...
        impl_->specific_file_system_, impl_->table_schema_, options, impl_->cache_,
        impl_->format_table_, impl_->table_resources_);
    impl_->Reset();
    return ctx;
}
