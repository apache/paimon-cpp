...
Result<std::unique_ptr<FileStoreScan>> AppendOnlyFileStoreWrite::CreateFileStoreScan(
    ...
    PAIMON_ASSIGN_OR_RAISE(
        ...
        AppendOnlyFileStoreScan::Create(snapshot_manager_, schema_manager_, manifest_list,
                                        manifest_file, table_schema_, schema_, scan_filter,
                                        options_, executor_, nullptr, pool_));
    return scan;
}
