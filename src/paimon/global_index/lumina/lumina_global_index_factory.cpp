...
Result<std::unique_ptr<GlobalIndexer>> LuminaGlobalIndexFactory::Create(
    const std::map<std::string, std::string>& options) const {
    return std::make_unique<LuminaGlobalIndex>(options);
}

REGISTER_PAIMON_FACTORY(LuminaGlobalIndexFactory);
