...
Result<std::unique_ptr<ReaderBuilder>> DataFileReaderFactory::CreateReaderBuilder(
    ...
    // Runtime read state rather than more format options: parquet turns its own pre-buffering
    // off when the shared read-ahead cache is reading ahead.
    ...
    if (read_options.prefetch_enabled && FormatSupportsPrefetch(format_identifier)) {
        PAIMON_ASSIGN_OR_RAISE(
            ...
            PrefetchFileBatchReaderImpl::Create(
                ...
                read_options.cache_config, read_options.prefetch_io_metrics_enabled,
