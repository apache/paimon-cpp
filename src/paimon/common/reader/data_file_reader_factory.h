...
///
/// Both table paths come through here - a managed table through `AbstractSplitRead`, a format
/// table through `FormatTableRead` - so which format reads a file, what it may cache and whether
/// it is read ahead are decided in one place rather than two that can drift.
