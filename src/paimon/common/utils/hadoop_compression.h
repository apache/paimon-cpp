...
///
/// Every name and extension below is fixed by what the rest of the paimon ecosystem writes, not
/// chosen here: `zstd` is spelled `zst` in a file name because that is the extension those files
/// carry. Changing one would not stop a parquet or orc file being read - both record their
/// compression inside - but it would break the naming convention shared with Java Paimon.
