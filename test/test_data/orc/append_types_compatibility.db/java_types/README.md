...
The two top-level precision-9 timestamp fields and the two precision-9 timestamp children of
f_row are written with microsecond data. Their complete nanosecond representation is shown with
the trailing three zeros (.123456000) instead of abbreviating it to .123456. VARIANT and VECTOR
are omitted because Java's ORC path does not support them.
