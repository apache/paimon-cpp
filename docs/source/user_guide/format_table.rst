...
* the ``csv``, ``json``, ``text``, ``lance``, ``mosaic`` and ``vortex`` file formats, leaving
  ``parquet`` and ``orc``. The first three are line-delimited text in Java, which shares one
  line-reading layer between them; this library has no text file format at all, so the first of
  them to be added has to bring that layer with it. ``lance``, ``mosaic`` and ``vortex`` are
  neither: each has a reader and a writer of its own, which this library builds under
  ``PAIMON_ENABLE_LANCE``, ``PAIMON_ENABLE_MOSAIC`` and ``PAIMON_ENABLE_VORTEX`` respectively, but
  a format table does not reach them yet;
